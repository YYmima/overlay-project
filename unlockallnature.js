function unlockAllNature() {
    ...
    for (let i = 1; i <= 1025; i++) {
        ...
        if (scene.gameData.dexData[i].caughtAttr !== 0n) {
            // natureAttr을 67108862로 설정, 기존 natureAttr이 이 값이 아니면 caughtCount를 25 추가
            if (scene.gameData.dexData[i].natureAttr !== 67108862) {
                scene.gameData.dexData[i].natureAttr = 67108862;
                scene.gameData.dexData[i].caughtCount = (scene.gameData.dexData[i].caughtCount || 0) + 25;
            }
        }
    }
